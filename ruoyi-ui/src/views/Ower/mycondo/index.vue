    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['Ower:mycondo:remove']"
        >delete</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['Ower:mycondo:export']"
        >export</el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="mycondoList" @selection-change="handleSelectionChange">
...
<script>
import { listMycondo, getMycondo, delMycondo, addMycondo, updateMycondo, exportMycondo } from "@/api/Ower/mycondo";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMycondo(this.queryParams).then(response => {
        this.mycondoList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != undefined) {
            updateMycondo(this.form).then(response => {
            ...
            addMycondo(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有前台管理数据项?', "警告", {
          ...
          return exportMycondo(queryParams);
        }).then(response => {
